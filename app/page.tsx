...
const funnel = [
  ...
  ["Сделка", "Скрипты и CRM не помогают закрывать"],
  ["Управление", "Решения принимаются по ощущениям"],
];

const beforeAfter = [
  ["До", "Ручной контроль, хаотичная CRM, слабая отчётность, зависимость от отдельных менеджеров."],
  ["После", "Роли, KPI, скрипты, аналитика, мотивация и регулярное управление собраны в единую систему."],
];

const steps = [
  ["01", "Диагностика", "Находим потери в воронке, CRM, команде и управлении."],
  ["02", "Архитектура", "Проектируем структуру отдела, роли, KPI, отчётность и стандарты."],
  ["03", "Внедрение", "Настраиваем процессы, обучаем команду и внедряем управленческий ритм."],
  ["04", "Рост", "Отслеживаем конверсии, устраняем потери и доводим систему до результата."],
];

export default function Home() {
  const [form, setForm] = useState({ name: "", contact: "", niche: "", problem: "" });
  ...
  return (
    <main className="min-h-screen bg-[#03190C] text-white">
      <section className="relative overflow-hidden px-6 py-8 md:px-12 lg:px-20">
        <div className="absolute inset-0 bg-[linear-gradient(115deg,#03190C_0%,#03190C_54%,#0B2D18_100%)]" />
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 0.16, scale: 1 }}
          transition={{ duration: 1.2 }}
          className="absolute -right-32 top-20 h-[560px] w-[560px] rounded-full bg-white blur-3xl"
        />
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="grid min-h-[78vh] gap-16 py-20 md:grid-cols-[1.15fr_0.85fr] md:items-center">
            <motion.div
              variants={fadeUp}
              initial="hidden"
              animate="visible"
            ...
            >
              ...
              <p className="mt-8 max-w-2xl text-lg leading-8 text-white/68">
                Выстраиваем отдел продаж, CRM, аналитику, скрипты, мотивацию
                и управление командой — чтобы бизнес рос не за счёт хаоса
                и ручного контроля, а за счёт системы.
              </p>

              <div className="mt-10 flex flex-col gap-4 sm:flex-row">
                <a href="#lead" className="rounded-full bg-white px-8 py-4 text-center font-medium text-[#03190C] transition hover:scale-[1.02]">
                  Получить диагностику
                </a>
                <a href="#case" className="rounded-full border border-white/20 px-8 py-4 text-center text-white/80 transition hover:bg-white/10">
                  Смотреть кейс
                </a>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="border-l border-white/10 pl-6 md:pl-10"
            >
      ...
      <AnimatedSection className="bg-[#F6F3EC] px-6 py-24 text-[#03190C] md:px-12 lg:px-20">
        <div className="mx-auto max-w-7xl">
          ...
          <h2 className="mt-5 max-w-4xl font-serif text-4xl leading-tight md:text-6xl">
            Деньги теряются не “в продажах”, а в конкретных переходах воронки
          </h2>
          ...
          <div className="mt-14 grid gap-px overflow-hidden border border-[#03190C]/10 bg-[#03190C]/10 md:grid-cols-5">
            {funnel.map(([title, text], index) => (
              ...
              >
                <div className="font-serif text-5xl text-[#03190C]/20">0{index + 1}</div>
          ...
          <div className="mt-14 grid gap-px overflow-hidden border border-white/10 bg-white/10 md:grid-cols-2">
            {beforeAfter.map(([title, text]) => (
              <div key={title} className="bg-[#03190C] p-8 md:p-10">
                <div className="text-sm uppercase tracking-[0.28em] text-white/35">{title}</div>
                <p className="mt-8 max-w-xl text-2xl leading-10 text-white/78">{text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <AnimatedSection id="case" className="bg-white px-6 py-24 text-[#03190C] md:px-12 lg:px-20">
          ...
          <div className="border border-[#03190C]/10">
            ...
            <div className="bg-[#F6F3EC] p-8 md:p-10">
              ...
              <blockquote className="mt-6 font-serif text-3xl leading-tight text-[#03190C]">
                “После внедрения стало понятно, где мы теряем клиентов и как управлять отделом не на ощущениях, а по цифрам.”
              </blockquote>
              <p className="mt-5 text-[#03190C]/55">Из интервью с клиентом проекта</p>
            </div>
          </div>
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection id="lead" className="px-6 py-24 md:px-12 lg:px-20">
        <div className="mx-auto grid max-w-7xl gap-12 md:grid-cols-[0.9fr_1.1fr]">
          ...
          >
            ...
            {status === "success" && <p className="mt-4 text-sm text-[#03190C]">Заявка отправлена. Мы скоро свяжемся с вами.</p>}
            {status === "error" && <p className="mt-4 text-sm text-red-600">Не удалось отправить заявку. Попробуйте ещё раз.</p>}
          </motion.form>
        </div>
      </AnimatedSection>
    </main>
...
function SectionLabel({ children, dark }: { children: React.ReactNode; dark?: boolean }) {
  return (
    <div className={`text-sm uppercase tracking-[0.28em] ${dark ? "text-[#03190C]/45" : "text-white/35"}`}>
      {children}
    </div>
  );
}
...
function Metric({ value, label }: { value: string; label: string }) {
  return (
    <motion.div whileHover={{ x: 6 }} className="border-b border-white/10 pb-6">
